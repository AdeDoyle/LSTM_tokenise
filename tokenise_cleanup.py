...
from preprocess_data_cleanup import load_conllu, map_chars, load_data, sequence, encode, onehot_split, split_on_latin
import os
        ...
        for lstmsize in LSTM_sizes:
            for denselayer in Dense_layers:
                for densesize in Dense_sizes:
                    if denselayer != 0:
                        NAME = f"{name_prefix}{name_infix}clean{name_suffix}, {lstmlayer}x{lstmsize} LSTMs, " \
                               f"{denselayer}x{densesize} Dense, {num_epochs} Epochs"
                        models_dir = os.getcwd() + "\\models\\clean_models"
                        try:
                        ...
                        if NAME not in models_list:
                            ...
                            print(model.summary())
                            # Log the model
                            tb = TensorBoard(log_dir=f"logs\clean_logs\{NAME}")
                            # Compile model
                            ...
                            # Save Model
                            model.save(f"models\clean_models\{NAME}")
...
# *Terminal*>tensorboard --logdir=logs\clean_logs\
